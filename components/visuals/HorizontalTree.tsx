...
// Safe transition wrapper with try-catch fallback
function safeTransition(sel: any, duration: number): any {
    try {
        ...
        console.warn('[HorizontalTree] transition() call failed, using instant fallback:', e);
    }
...
export function HorizontalTree() {
    ...
    const duration = TREE_ANIMATION_CONFIG.DURATION.HORIZONTAL
    const nodeWidth = 240
    const nodeHeight = 90
    ...
    useEffect(() => {
        ...
        // Initially collapse root - ONLY root node visible
        // User clicks root to see first-level children, click those to see grandchildren
        collapse(root)
        ...
        function update(source: HierarchyNode) {
            ...
            safeTransition(nodeUpdate, duration)
                .attr("transform", (d: HierarchyNode) => `translate(${d.y},${d.x})`)

            safeTransition(nodeUpdate.select('rect'), duration)
                .style("fill", (d: any) => d._children ? "#2b2f36" : "#1e1f23")
                .style("stroke", "#2e4b43")
                .style("opacity", 1)

            safeTransition(nodeUpdate.select('text'), duration)
                .style("fill-opacity", 1)

            nodeUpdate.select('.indicator')
                .text((d: any) => d._children ? '>' : (d.children ? '<' : ''))

            safeTransition(nodeUpdate.select('.indicator'), duration)
                .style("opacity", (d: any) => (d.children || d._children) ? 1 : 0)

            // Exit
            const exitSel = node.exit()
            safeTransition(exitSel, duration)
                .attr("transform", ((d: HierarchyNode) => `translate(${source.y},${source.x})`) as any)
                .remove()
            exitSel.select('rect').style('opacity', 0)
            exitSel.select('text').style('fill-opacity', 0)
            exitSel.select('.indicator').style('opacity', 0)
            ...
            safeTransition(linkMerged, duration)
                .attr('d', (d: any) => diagonal(d.source as any, d.target as any))
                .style("stroke-opacity", 0.35)

            safeTransition(link.exit(), duration)
                .attr('d', (d: any) => {
            ...
            function mouseover(event: any, d: HierarchyNode) {
                ...
                safeTransition(rect as any, 200)
                    .style("stroke", "#4fd1c5")
