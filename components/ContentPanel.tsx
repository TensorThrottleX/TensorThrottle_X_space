...
export function ContentPanel({ children, title, subtitle, latestPublishedAt }: ContentPanelProps) {
  ...
  return (
    <div className="flex flex-1 items-center justify-center px-4 py-8 sm:px-6 lg:px-8 h-screen w-full">
      ...
      <div
        className="relative w-full max-w-2xl h-[92vh] rounded-3xl backdrop-blur-2xl backdrop-saturate-150 border shadow-2xl overflow-hidden flex flex-col transition-all duration-500 hover:shadow-cyan-500/5"
        style={{
      ...
      >
        {/* Header (optional) */}
        {title && (
          <div className="px-10 py-10 shrink-0 border-b transition-colors duration-500"
        ...
        <div className="flex-1 overflow-y-auto px-10 py-10 premium-scrollbar scroll-smooth">
          {children}
        </div>
      </div>
    </div>
